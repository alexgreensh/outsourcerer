...
**Real names, not hashes.** `fleet name` reads what each session is actually working on and names it, so `session-3f9a` reads as `Refactor the auth retry logic`, cached so the list stays instant.
