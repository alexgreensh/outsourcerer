...
| any model the USER configured | **droid** / **cursor** / **warp** / **hermes** / **cline** (`--provider droid\|cursor\|warp\|hermes\|cline`) — the user's OWN agent CLIs, THEIR models (incl. BYOK); `-m` passes through verbatim. Warp drives `oz agent run` (models via `oz model list`) and can even host the Claude/Codex harness (`OSRC_WARP_HARNESS=claude\|codex`). **Cline** rides the FREE `cline` OAuth provider (`cline auth cline`) which serves `deepseek/deepseek-v4-flash` + `z-ai/glm-5.2` at $0 cash — pass either via `-m`. **These are easy to forget — the user has warp AND droid AND cline installed; when they say "use warp/droid/cline," this is the lane, don't reach for devin.** |
